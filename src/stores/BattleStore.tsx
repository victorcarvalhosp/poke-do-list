...
export class BattleStore implements IBattleStore {
  ...
  @observable
  attackMessages: IMoveMessages = new MoveMessages(
    "",
    0,
    "average",
    false,
  ...
  initBattle() {
    ...
    this.activePos = 0;
    const initOpponentPosSelected =
      this.player2SelectedPokemons[1].actualHp > 0 ? 1 : 0;
    this.battleResult = "";
    ...
    this.player1TurnAction = [
      new BattleAction(1, 0, 1, this.player1SelectedPokemons[0].moves[0]),
      new BattleAction(
        1,
        1,
        initOpponentPosSelected,
        this.player1SelectedPokemons[1].moves[0]
      ),
      new BattleAction(
        1,
        2,
        initOpponentPosSelected,
        this.player1SelectedPokemons[2].moves[0]
      ),
    ];
    this.player2TurnAction = [
      new BattleAction(2, 0, 0, this.player2SelectedPokemons[0].moves[0]),
      new BattleAction(2, 1, 0, this.player2SelectedPokemons[1].moves[0]),
      new BattleAction(2, 2, 0, this.player2SelectedPokemons[2].moves[0]),
    ];
  }
  ...
  setOpponent(trainer: ITrainerInfo) {
    ...
    this.player2SelectedPokemons[0] = this.root.pokemonStore.generatePokemonWithRandomAttributes(
      this.opponentInfo.pokemon[0].variety,
      "",
      this.opponentInfo.pokemon[0]
    );
    this.player2SelectedPokemons[1] = this.root.pokemonStore.generatePokemonWithRandomAttributes(
      this.opponentInfo.pokemon[1].variety,
      "",
      this.opponentInfo.pokemon[1]
    );
    this.player2SelectedPokemons[2] = this.root.pokemonStore.generatePokemonWithRandomAttributes(
      this.opponentInfo.pokemon[2].variety,
      "",
      this.opponentInfo.pokemon[2]
    );
  }
  ...
  async runTurn() {
    if (
      ...
      const allActions: IBattleAction[] = [
        ...this.player1TurnAction,
        ...this.player2TurnAction,
      ];
      const allActionsOrderedBySpeed: IBattleAction[] = allActions.sort(
        (a, b) =>
          (a.player === 1
            ? this.player1SelectedPokemons[a.pos].speed
            : this.player2SelectedPokemons[a.pos].speed) <
          (b.player === 1
            ? this.player1SelectedPokemons[b.pos].speed
            : this.player2SelectedPokemons[b.pos].speed)
            ? 1
            : -1
      );
  ...
  private async runAction(
    ...
    const attackerTeam =
      action.player === 1
        ? this.player1SelectedPokemons
        : this.player2SelectedPokemons;
    const defenderTeam =
      action.player === 1
        ? this.player2SelectedPokemons
        : this.player1SelectedPokemons;
    ...
    if (attacker && attacker.actualHp > 0) {
      this.attackMessages.text =
        `${action.player === 1 ? "My " : "Enemy "}` +
        pokemonVarieties[attacker.variety].name +
        " used " +
        moves[action.move].name +
        " on " +
        pokemonVarieties[defender.variety].name;
      const damage: number = this.calculateDamage(
        attacker,
        defender,
        moves[action.move]
      );
  ...
  private getNextAvailableAttackPosition(defenderTeam: IPokemon[]): number {
    return defenderTeam[0] && defenderTeam[0].actualHp > 0
      ? 0
      : defenderTeam[1] && defenderTeam[1].actualHp > 0
      ? 1
      : 2;
  }
  ...
  private refreshTurnAction() {
    this.setActivePos(
      this.getNextAvailableAttackPosition(this.player1SelectedPokemons)
    );
    this.player1TurnAction.forEach((action, i) => {
      action.pos = i;
      if (this.player2SelectedPokemons[action.opponentPos].actualHp <= 0) {
        action.opponentPos = this.getNextAvailableAttackPosition(
          this.player2SelectedPokemons
        );
      }
    });
  ...
  private player1StillHaveLivePokemon() {
    return (
      !this.isPokemonPosDead(1, 0) ||
      !this.isPokemonPosDead(1, 1) ||
      !this.isPokemonPosDead(1, 2)
    );
  }

  private player2StillHaveLivePokemon() {
    return (
      !this.isPokemonPosDead(2, 0) ||
      !this.isPokemonPosDead(2, 1) ||
      !this.isPokemonPosDead(2, 2)
    );
  }
  ...
  private async runAnimation(
    ...
    if (player === 1) {
      ...
      // @ts-ignore
      document
        .getElementById(`pkmn-p1-${pos}`)
        .classList.add("class", `animation-atk-player1`);
      // @ts-ignore
      document
        .getElementById(`pkmn-p1-${pos}`)
        .classList.add("class", `enemy-pos-${opponentPos}`);
      if (this.player2SelectedPokemons[opponentPos].gigantamax) {
        // @ts-ignore
        document
          .getElementById(`pkmn-p1-${pos}`)
          .classList.add("class", `animation-atk-gigantamax`);
      }
      await addDelay(1000);
      ...
      document
        .getElementById(`pkmn-p1-${pos}`)
        .classList.remove(`animation-atk-player1`);
      // @ts-ignore
      document
        .getElementById(`pkmn-p1-${pos}`)
        .classList.remove(`enemy-pos-${opponentPos}`);
      // @ts-ignore
      if (this.player2SelectedPokemons[opponentPos].gigantamax) {
        // @ts-ignore
        document
          .getElementById(`pkmn-p1-${pos}`)
          .classList.remove(`animation-atk-gigantamax`);
      }
    } else {
      await addDelay(1000);
      ...
      // @ts-ignore
      document
        .getElementById(`pkmn-p2-${pos}`)
        .classList.add("class", `animation-atk-player2`);
      // @ts-ignore
      document
        .getElementById(`pkmn-p2-${pos}`)
        .classList.add("class", `enemy-pos-${opponentPos}`);
      ...
      document
        .getElementById(`pkmn-p2-${pos}`)
        .classList.remove(`animation-atk-player2`);
      // @ts-ignore
      document
        .getElementById(`pkmn-p2-${pos}`)
        .classList.remove(`enemy-pos-${opponentPos}`);
  ...
  private calculateDamage(attacker: IPokemon, defender: IPokemon, move: IMove) {
    ...
    const sameTypeAttackBonus =
      move.type === pokemonVarieties[attacker.variety].type1 ||
      move.type === pokemonVarieties[attacker.variety].type2
        ? 1.5
        : 1;
    const typeModifier: number = this.calculateTypeModifier(
      move.type,
      pokemonVarieties[defender.variety].type1,
      pokemonVarieties[defender.variety].type2
    );
    ...
    critical > 1
      ? (this.attackMessages.critical = true)
      : (this.attackMessages.critical = false);
    const modifier =
      getRandomNumber(0.85, 1) * sameTypeAttackBonus * typeModifier * critical;
  ...
  private calculateTypeModifier(
    ...
    const typeModifier =
      1 *
      typeChart[attackType][type1] *
      (type2 ? 1 * typeChart[attackType][type2] : 1);
    console.log(
      attackType + " vs " + type1 + "/" + type2 + "===" + typeModifier
    );
    return typeModifier;
  }
  ...
  get myPokemonListWithMaxLevel() {
    return this.opponentInfo && this.opponentInfo.maxLevel
      ? this.root.pokemonStore.listOrdered.filter(
          (pkmn) => pkmn.level <= (this.opponentInfo.maxLevel || 100)
        )
      : this.root.pokemonStore.listOrdered;
  }
  ...
  get battleIsHappening() {
    return (
      this.player1SelectedPokemons[0].id !== "" &&
      this.player1SelectedPokemons[1].id !== "" &&
      this.player1SelectedPokemons[2].id !== ""
    );
  }
}
