...
const BattleSelectPokemonPage: React.FC<RouteComponentProps> = observer(
  ({ history }) => {
    ...
    }, [battleStore, pokemonStore]);

    const handleSelectPokemon = (pokemonSelected: IPokemon) => {
      battleStore.player1SelectedPokemons.filter(
        (pkmn) => pkmn.id === pokemonSelected.id
      ).length > 0
        ? openModalDetails(pokemonSelected)
        : battleStore.setPokemonPos(selectedPos, pokemonSelected);
    ...
    return (
      <IonPage id="battle-select-pokemon-page">
        ...
        <IonContent scrollEvents={true} className="ion-padding">
            ...
            <IonRow className="ion-align-items-center ion-justify-content-center">
              <div className="opponent-info">
                {battleStore.opponentInfo.sprite && (
                  <Overworld
                    spriteUrl={
                      battleStore.opponentInfo.sprite || "invisible.png"
                    }
                    direction="down"
                    type="human"
                ...
                <p>{battleStore.opponentInfo.name}</p>
                {battleStore.opponentInfo.maxLevel && (
                  <span>Max lv: {battleStore.opponentInfo.maxLevel}</span>
                )}
                {battleStore.opponentInfo.type && (
                  <Type type1={battleStore.opponentInfo.type} />
                )}
              </div>
            </IonRow>
          </IonGrid>
          <div className="selected-pokemons">
            ...
            <IonGrid>
              <IonRow className="ion-align-items-center ion-justify-content-center">
                {[0, 1, 2].map((i) => (
                  <PkmnGridItem
                    pkmn={battleStore.player1SelectedPokemons[i]}
          ...
          <PkmnList
            list={battleStore.myPokemonListWithMaxLevel}
            handleItemClick={handleSelectPokemon}
          />
          <BattleSelectPokemonDetailsModal
        ...
        <IonButton
          ...
          expand="full"
          disabled={
            battleStore.player1SelectedPokemons.filter((pkmn) => pkmn.id !== "")
              .length < 3
          }
          onClick={goToBattle}
        >
